...
- **Append-only** — never edit or delete existing entries. To change a decision: add a new entry (next D-number) with `Supersedes: D-XXX` and update the old entry's `Status` to `Deprecated → D-XXX`.
- **Conflict rule**: if your implementation would contradict an `Active` entry, stop — surface the conflict to the user before writing any code or creating a new entry.
